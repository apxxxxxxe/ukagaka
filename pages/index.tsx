...
export class Commit {
  ...
  constructor(repoName: string, date: string, messages: string[]) {
  ...
  render() {
    return (
      <div className="ml-5" key={`commit-${this.date}-${this.repoName}}`}>
        <h3 className="font-bold mb-1">
          <span
            className="mr-1"
            style={{
              color: `#${pieces.find(
                (piece) => piece.repoName === this.repoName
              )?.color
                }`,
            }}
          >
...
export class Release {
  ...
  constructor(repoName: string, date: string, tagName: string, body: string) {
    this.repoName = repoName
    this.date = date
  ...
  render() {
    return (
      <div className="ml-5" key={`release-${this.date}-${this.tagName}`}>
        <h2 className="text-xl font-bold mb-1">
          ...
          <Link
            href={`https://github.com/apxxxxxxe/${this.repoName}/releases/tag/${this.tagName}`}
            className="grow font-bold hover:underline"
          >
            {`${pieceNameByRepoName(this.repoName)} Release ${this.tagName
              }`}
          </Link>
        </h2>
        <div className="ml-3 my-3">
          {this.bodyHtml !== null
            ? makeGitHubReleaseDescription(this.bodyHtml)
            : this.body}
        </div>
      </div>
    )
...
const Page: NextPage = ({ pushedAts }: { pushedAts: Repository[] }) => {
  ...
  useEffect(() => {
    const fetchFromDB = async () => {
      ...
      const [c, r] = (await Promise.all(promises)).flat()
      setCommits(c)
      setReleases(r)
      setIsFetching(false)
